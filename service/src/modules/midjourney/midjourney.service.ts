...
export class MidjourneyService {
  constructor(
    ...
    @InjectRepository(MjPromptsEntity)
    private readonly mjPromptsEntity: Repository<MjPromptsEntity>,
  ...
  async updateDrawData(jobData, drawRes) {
    try {
      ...
      console.log('mjNotSaveImg: ', mjNotSaveImg);
      let cosUrl = ''
  ...
  formatFileInfo(fileInfo, isSaveImg,  mjProxyImgUrl, originUrl) {
    ...
    const parseFileInfo = JSON.parse(fileInfo);
