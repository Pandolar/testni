...
export interface NineStoreInterface {
  ...
  getUuid(): string
  buildMessageFromParentMessageId(string, opt?: BuildMessageOptions): Promise<BuildMessageRes>;
}
...
export class NineStore implements NineStoreInterface {
  ...
  public async buildMessageFromParentMessageId(text: string, options: BuildMessageOptions){
    ...
    // let parentId = '1bf30262-8f25-4a03-88ad-9d42d55e6f0b'
      ...
      if(maxModelToken && maxResponseTokens){
        ...
        if(!isValidPrompt){
          nextMessages = this._recursivePruning(messages, maxNumTokens, systemMessage)
        }
      }
  ...
  protected async _getTokenCount(messages: any[]) {
    let text = messages.reduce( (pre: string, cur: any) => {
      return pre+=cur.content
    }, '')
    console.log('汇总的文本数量: ', text);
  ...
  protected async _recursivePruning(
    ...
    const currentTokens = await this._getTokenCount(messages)
