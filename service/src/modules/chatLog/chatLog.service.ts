...
export class ChatLogService {
  ...
  async querAllDrawLog(params: QuerAllDrawLogDto) {
    ...
    const where = { type: DeductionKey.PAINT_TYPE, prompt: Not(''), answer: Not('') };
  ...
  async chatList(req: Request, params: ChatListDto) {
    ...
    return list.map((item) => {
      ...
      return {
        ...
        inversion: role === 'user',
        error: false,
        conversationOptions: conversationOptions ? JSON.parse(conversationOptions) : null,
        requestOptions: requestOptions ? JSON.parse(requestOptions) : null,
      };
    });
  }
